...
const claims: Record<string, PaidAdmissionClaims> = ["one", "two", "three", "four", "five", "six"].reduce((all, token, index) => ({
  ...all,
...
describe("paid arena runtime", () => {
  it("admits only consumed signed tickets and produces a wallet-free immutable result", async () => {
    const runtime = new PaidArenaRuntime({
      ...
      arenaConfig: { minPlayersToStart: 6, maxPlayers: 6, countdownDurationMs: 10, finishedDurationMs: 5, resultsDurationMs: 10, freeModeBotsEnabled: true }
    });
    ...
    await runtime.admit({ sessionId: "session-4", token: "four" });
    await runtime.admit({ sessionId: "session-5", token: "five" });
    await runtime.admit({ sessionId: "session-6", token: "six" });
  ...
  it("never consumes an admission after the paid roster is frozen for countdown", async () => {
    const runtime = new PaidArenaRuntime({
      matchId: "match-1", roundId: "round-1", admissionConsumer: new FakeConsumer(),
      arenaConfig: { minPlayersToStart: 6, maxPlayers: 6, countdownDurationMs: 10 }
    });
    await runtime.admit({ sessionId: "session-1", token: "one" });
    await runtime.admit({ sessionId: "session-2", token: "two" });
    await runtime.admit({ sessionId: "session-3", token: "three" });
  ...
  it("publishes one immutable wallet-free result only after the authoritative round ends", async () => {
    ...
    const adapter = new PaidArenaTransportAdapter(new PaidArenaRuntime({
      matchId: "match-1", roundId: "round-1", admissionConsumer: new FakeConsumer(),
      arenaConfig: { minPlayersToStart: 6, maxPlayers: 6, countdownDurationMs: 10, finishedDurationMs: 5 }
    ...
    await adapter.join("session-4", "four"); await adapter.join("session-5", "five"); await adapter.join("session-6", "six");
  ...
  it("returns the common authoritative input rejection instead of accepting a paid-only bypass", async () => {
    const runtime = new PaidArenaRuntime({ matchId: "match-1", roundId: "round-1", admissionConsumer: new FakeConsumer() });
    expect(runtime.trySetInput("unknown-session", { x: 1, y: 0 }, 0)).toEqual({
      accepted: false,
      reason: ArenaInputRejectionReason.PLAYER_NOT_FOUND,
    });
  });
});
