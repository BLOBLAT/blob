...
bounded internal entry/player/match/round IDs, a SHA-256 rules hash, a UUID
nonce, a 10-second-to-five-minute issued/expiry window, and an Ed25519
signature. The Platform API retains its private signing key; a future game
server receives only its public verification key. Wallet addresses
